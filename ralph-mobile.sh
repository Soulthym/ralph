...
if [[ -z "$PORT" ]]; then
  print_prefixed "SYSTEM" "Failed to start opencode server" >&2
  print_prefixed "SYSTEM" "$(cat "$SERVER_OUTPUT")" >&2
  exit 1
fi
    ...
    if [[ "$line" == data:* ]]; then
      ...
      if [[ "$EVENT_TYPE" == "message.part.updated" ]]; then
        ...
        if [[ "$PART_SESSION" == "$SESSION_ID" && "$PART_TYPE" == "text" ]]; then
          ...
          COMMAND=$(echo "$DATA" | jq -r '.properties.part.state.input.command // empty' 2>/dev/null) || true
          TOOL_TITLE="$TITLE"
          ...
          if [[ "$TOOL_STATUS" == "completed" ]]; then
            TOOL_OUTPUT=$(echo "$DATA" | jq -r '.properties.part.state.output // empty' 2>/dev/null) || true
            TOOL_ERROR=$(echo "$DATA" | jq -r '.properties.part.state.error // empty' 2>/dev/null) || true
